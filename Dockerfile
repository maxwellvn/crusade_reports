...
RUN mkdir -p data
EXPOSE 4000
HEALTHCHECK --interval=30s --timeout=5s --start-period=180s CMD ["node", "-e", "fetch(`http://127.0.0.1:${process.env.PORT || 4000}/api/health`).then(r => { if (!r.ok) process.exit(1) }).catch(() => process.exit(1))"]
