...
export const missionNationSelectionSchema = z.object({
  ...
  mission_country_codes: z.array(z.string().trim().length(2).toUpperCase()).min(1, "Select at least one mission nation").max(241),
  contact_email: z.string().trim().email("Enter a valid email address").max(254),
  ...
  if (new Set(data.mission_country_codes).size !== data.mission_country_codes.length) ctx.addIssue({ code: z.ZodIssueCode.custom, path: ["mission_country_codes"], message: "Each mission nation can only be selected once" });
  if (data.mission_country_codes.includes(data.home_country_code)) ctx.addIssue({ code: z.ZodIssueCode.custom, path: ["mission_country_codes"], message: "Choose nations outside your home nation" });
});
...
export const upcomingCrusadeInterestSchema = z.object({
  ...
  group_name: z.string().trim().max(250).optional().default(""),
  passport_country_code: z.string().length(2, "Select your passport country").toUpperCase(),
  opportunity_codes: z.array(z.string().min(2).max(20).toUpperCase()).min(1, "Select at least one upcoming crusade").max(50, "Select no more than 50 crusades").refine((codes) => new Set(codes).size === codes.length, "Select each crusade only once"),
  additional_information: z.string().trim().max(2000).optional().default(""),
});
