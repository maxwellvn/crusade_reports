  ...
  for (const part of ["sponsored", "own"]) {
    data[part].forEach((crusade, index) => {
      const row = insertCrusade.run({
        ...
        event_date: crusade.event_date, attendance: crusade.attendance, currency_code: crusade.currency_code,
...
const lineColumns = [
  ...
  { header: "Accompanying flights", value: (r) => r.accompanying_flight },
