...
export function mediaTrainingRows(query = {}) {
  ...
  if (q) {
    where.push(`(r.reference_code LIKE @q OR r.zone_name LIKE @q OR r.group_name LIKE @q OR r.church_name LIKE @q
  ...
  return db.prepare(`SELECT r.id AS registration_id, r.reference_code, r.zone_name, r.group_name, r.church_name,
    ...
    t.id AS trainee_id, t.full_name, t.role, t.email, t.kingschat_username, t.phone_country_code, t.phone_number
...
mediaTraining.post("/registrations", wrap(async (req, res) => {
  ...
  db.transaction(() => {
    const registrationId = db.prepare("INSERT INTO media_training_registrations (reference_code, zone_name, group_name, church_name, organization_name, primary_timezone) VALUES (?, ?, ?, ?, ?, '')")
