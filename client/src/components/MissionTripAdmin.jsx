...
export function MissionTripAdmin() {
  const [data, setData] = React.useState(null); const [query, setQuery] = React.useState(""); const [zone, setZone] = React.useState(""); const [passport, setPassport] = React.useState(""); const [destination, setDestination] = React.useState(""); const [sort, setSort] = React.useState("created_at:desc");
  const params = React.useMemo(() => { const p = new URLSearchParams(); if (query.trim()) p.set("q", query.trim()); if (zone) p.set("zone", zone); if (passport) p.set("passport", passport); if (destination) p.set("destination", destination); const [field, direction] = sort.split(":"); p.set("sort", field); p.set("direction", direction); return p; }, [query, zone, passport, destination, sort]);
  ...
  function exportPdf() {
    ...
    const html = buildNotcReportHtml({
      ...
      summary: [
        { label: "Applications", value: nfull.format(data.total) },
