...
function CrusadeCard({ part, index, register, control, setValue, errors, crusade, onRemove, files, setFiles, fetchCountries, cityFetcherFor }) {
...
export function CrusadeExpenses() {
  ...
  const { register, control, handleSubmit, watch, reset, setError, setValue, formState: { errors, isSubmitting } } = useForm({ resolver: zodResolver(zoneExpenseReportSchema), defaultValues: defaults });
  ...
  return <div className="min-h-screen bg-[#f6f2e8] text-slate-950"><header className="border-b border-black/10 bg-white"><div className="mx-auto flex max-w-7xl items-center gap-3 px-5 py-4"><Link to="/"><img src="/logo.png" alt="A Night of a Thousand Crusades" className="h-11" /></Link><span className="hidden min-w-0 truncate text-sm font-semibold sm:block">Zonal Crusade Expense Report</span><Link to="/" className="ml-auto shrink-0 text-sm font-semibold">Return home</Link></div></header>
    ...
    <form onSubmit={handleSubmit(submit, () => toast.error("Check the highlighted details."))} className="bg-white px-5 py-12 sm:px-10 lg:px-14 lg:py-16"><div className="max-w-3xl"><p className="text-sm font-semibold text-amber-700">{editing ? `Editing · submitted ${editing.created_at.slice(0, 10)}` : "Zonal expense report"}</p><h2 className="mt-3 text-3xl font-medium leading-tight tracking-[-0.035em] sm:text-4xl">Crusade expenses for your zone</h2><p className="mt-4 max-w-2xl leading-7 text-slate-600">One consolidated report per zone, in two parts: the crusades you were invited to, and the mega crusades your zone planned and held.</p>
      ...
      <section className="mt-14"><div className="flex gap-3"><Plane className="mt-1 size-5 text-amber-600" /><div><p className="text-xs font-semibold uppercase tracking-[0.16em] text-amber-700">Part A</p><h3 className="mt-1 text-2xl font-medium tracking-[-0.02em]">Crusades you were invited to</h3><p className="mt-1 text-sm leading-6 text-slate-600">One entry per crusade you travelled for: flights, what your zone has already given toward the sponsorship, and any other costs.</p></div></div>
        <ul className="mt-6 border-y border-black/15">{sponsored.fields.length === 0 ? <li className="py-8 text-sm text-slate-500">No crusades added yet.</li> : sponsored.fields.map((row, index) => <CrusadeCard key={row.id} part="sponsored" index={index} register={register} control={control} setValue={setValue} errors={errors} fetchCountries={fetchCountries} cityFetcherFor={cityFetcherFor} crusade={values.sponsored?.[index]} onRemove={() => removeCrusade("sponsored", index, sponsored)} files={filesFor("sponsored", index)} setFiles={setFilesFor("sponsored", index)} />)}</ul>
        <div className="flex flex-wrap items-center justify-between gap-4 py-5"><Button type="button" variant="outline" className="rounded-full transition-[transform,background-color] duration-150 active:scale-[0.96]" onClick={() => sponsored.append(emptySponsored())}><Plus /> Add a crusade you were invited to</Button><p className="text-sm text-slate-500">Part A total <span className="ml-2 font-semibold tabular-nums text-slate-950">E {money(sponsoredEspees)}</span></p></div></section>
      ...
      <section className="mt-10"><div className="flex gap-3"><Coins className="mt-1 size-5 text-amber-600" /><div><p className="text-xs font-semibold uppercase tracking-[0.16em] text-amber-700">Part B</p><h3 className="mt-1 text-2xl font-medium tracking-[-0.02em]">Mega crusades your zone held</h3><p className="mt-1 text-sm leading-6 text-slate-600">Crusades your zone planned and held — {MEGA_CRUSADE_MINIMUM.toLocaleString()} in attendance and above. Venue and transportation, with evidence.</p></div></div>
        <ul className="mt-6 border-y border-black/15">{own.fields.length === 0 ? <li className="py-8 text-sm text-slate-500">No crusades added yet.</li> : own.fields.map((row, index) => <CrusadeCard key={row.id} part="own" index={index} register={register} control={control} setValue={setValue} errors={errors} fetchCountries={fetchCountries} cityFetcherFor={cityFetcherFor} crusade={values.own?.[index]} onRemove={() => removeCrusade("own", index, own)} files={filesFor("own", index)} setFiles={setFilesFor("own", index)} />)}</ul>
