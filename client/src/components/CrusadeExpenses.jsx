...
function CrusadeCard({ part, index, count, register, control, setValue, errors, crusade, onRemove, files, setFiles, fetchCountries, cityFetcherFor }) {
  ...
  return <li className="border-t border-black/10 py-8 first:border-t-0"><div className="flex items-start justify-between gap-4"><p className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-500">{part === "sponsored" ? "Crusade" : "Mega crusade"}{count > 1 ? ` ${index + 1}` : ""}</p><Button type="button" variant="ghost" size="sm" className="text-slate-500 hover:text-red-700" onClick={onRemove}><Trash2 /> Remove</Button></div>
...
export function CrusadeExpenses() {
  ...
  return <div className="min-h-screen bg-[#f6f2e8] text-slate-950"><header className="border-b border-black/10 bg-white"><div className="mx-auto flex max-w-7xl items-center gap-3 px-5 py-4"><Link to="/"><img src="/logo.png" alt="A Night of a Thousand Crusades" className="h-11" /></Link><span className="hidden min-w-0 truncate text-sm font-semibold sm:block">Zonal Crusade Expense Report</span><Link to="/" className="ml-auto shrink-0 text-sm font-semibold">Return home</Link></div></header>
    ...
    <form onSubmit={handleSubmit(submit, () => toast.error("Check the highlighted details."))} className="bg-white px-5 py-12 sm:px-10 lg:px-14 lg:py-16"><div className="max-w-3xl"><p className="text-sm font-semibold text-amber-700">{editing ? `Editing · submitted ${editing.created_at.slice(0, 10)}` : "Zonal expense report"}</p><h2 className="mt-3 text-3xl font-medium leading-tight tracking-[-0.035em] sm:text-4xl">Crusade expenses for your zone</h2><p className="mt-4 max-w-2xl leading-7 text-slate-600">One consolidated report per zone, in two parts: the crusade the Rhapsody department invited you to, and the mega crusades your zone planned and held.</p>
      ...
      <section className="mt-14"><div className="flex gap-3"><Plane className="mt-1 size-5 text-amber-600" /><div><p className="text-xs font-semibold uppercase tracking-[0.16em] text-amber-700">Part A</p><h3 className="mt-1 text-2xl font-medium tracking-[-0.02em]">Crusade you were invited to by the Rhapsody department</h3><p className="mt-1 text-sm leading-6 text-slate-600">Usually one crusade. Give the flights, what your zone has already given toward the sponsorship, and any other costs. Add another only if you travelled for more than one.</p></div></div>
        <ul className="mt-6 border-y border-black/15">{sponsored.fields.length === 0 ? <li className="py-8 text-sm text-slate-500">No crusade added. Use the button below if the department invited your zone to one.</li> : sponsored.fields.map((row, index) => <CrusadeCard key={row.id} part="sponsored" index={index} count={sponsored.fields.length} register={register} control={control} setValue={setValue} errors={errors} fetchCountries={fetchCountries} cityFetcherFor={cityFetcherFor} crusade={values.sponsored?.[index]} onRemove={() => removeCrusade("sponsored", index, sponsored)} files={filesFor("sponsored", index)} setFiles={setFilesFor("sponsored", index)} />)}</ul>
        <div className="flex flex-wrap items-center justify-between gap-4 py-5"><Button type="button" variant="outline" className="rounded-full transition-[transform,background-color] duration-150 active:scale-[0.96]" onClick={() => sponsored.append(emptySponsored())}><Plus /> Add another crusade</Button><p className="text-sm text-slate-500">Part A total <span className="ml-2 font-semibold tabular-nums text-slate-950">E {money(sponsoredEspees)}</span></p></div></section>
      ...
      <section className="py-10"><Field label="Notes" hint="Anything the administration should know — including crusades still coming up."><Textarea rows={4} {...register("notes")} /></Field><div className="mt-8 flex flex-col-reverse gap-4 border-t border-black/10 pt-8 sm:flex-row sm:items-center sm:justify-between"><p className="max-w-md text-xs leading-5 text-slate-500">If the department did not invite your zone to a crusade, leave Part A empty. Reopen the report any time with your zone and KingsChat username.</p><Button type="submit" disabled={isSubmitting} className="rounded-full bg-[#18234d] px-7 transition-[transform,background-color] duration-150 hover:bg-[#24346d] active:scale-[0.96]">{isSubmitting ? "Saving…" : editing ? "Update expense report" : "Submit expense report"}</Button></div></section>
    </div></form></main></div>;
}
