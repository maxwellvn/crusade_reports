...
import { postForm, putForm, postJSON } from "@/lib/api";
import { EXPENSE_DESIGNATIONS, MEGA_CRUSADE_MINIMUM, zoneExpenseReportSchema } from "@/lib/schema";
import { useOrgData } from "@/lib/orgForm";

const espees = new Intl.NumberFormat("en", { minimumFractionDigits: 2, maximumFractionDigits: 2 });
const CURRENCIES = typeof Intl.supportedValuesOf === "function" ? Intl.supportedValuesOf("currency") : ["NGN", "USD", "GBP", "EUR", "ZAR", "GHS", "KES", "XOF", "XAF"];
...
const defaults = { zone_name: "", designation: "", first_name: "", last_name: "", kingschat_username: "", notes: "", sponsored: [], own: [] };
...
const toFormValues = (report) => ({
  ...
  sponsored: report.sponsored.map((c) => ({ ...emptySponsored(), ...c, city: c.city || "", other_cost_note: c.other_cost_note || "", note: c.note || "", keep_evidence: c.evidence.map((f) => f.id), evidence: c.evidence })),
...
function Confirmation({ result, editing }) {
  const rows = [["Zone", result.zone_name], ["Part A — sent by NOTC", `${result.sponsored.length} crusade${result.sponsored.length === 1 ? "" : "s"} · ES ${money(result.sponsored_espees)}`], ["Part B — your own", `${result.own.length} crusade${result.own.length === 1 ? "" : "s"} · ES ${money(result.own_espees)}`], ["Total", `ES ${money(result.total_espees)}`], ["Already given", `ES ${money(result.espees_already_given)}`], ["Reference", result.reference_code]];
...
function CrusadeCard({ part, index, register, control, errors, crusade, onRemove, files, setFiles }) {
  ...
  return <li className="border-t border-black/10 py-8 first:border-t-0"><div className="flex items-start justify-between gap-4"><p className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-500">{part === "sponsored" ? "Crusade" : "Mega crusade"} {index + 1}</p><Button type="button" variant="ghost" size="sm" className="text-slate-500 hover:text-red-700" onClick={onRemove}><Trash2 /> Remove</Button></div>
    <div className="mt-5 grid gap-5 sm:grid-cols-2">
      ...
      <Field label="Nation" required error={err.nation?.message}><Input {...register(`${part}.${index}.nation`)} placeholder="Nation where it was held" /></Field>
      <Field label="City" error={err.city?.message}><Input {...register(`${part}.${index}.city`)} placeholder="Optional" /></Field>
      <Field label="Date held" required error={err.event_date?.message}><Input type="date" {...register(`${part}.${index}.event_date`)} /></Field>
      <Field label="Attendance" required hint={`Mega crusades only — ${MEGA_CRUSADE_MINIMUM.toLocaleString()} and above`} error={err.attendance?.message}><Input type="number" inputMode="numeric" min={MEGA_CRUSADE_MINIMUM} step="1" className="tabular-nums" {...register(`${part}.${index}.attendance`)} /></Field>
      <Field label="Currency of these costs" required error={err.currency_code?.message}><Select {...register(`${part}.${index}.currency_code`)}><option value="">Select currency</option>{CURRENCIES.map((c) => <option key={c} value={c}>{c}</option>)}</Select></Field>
      ...
      <div className="grid gap-5 border-t border-black/10 pt-6 sm:col-span-2 sm:grid-cols-2">
        <div className="flex items-baseline justify-between gap-4 text-sm sm:col-span-2"><span className="text-slate-500">Actual cost in {code || "local currency"}</span><span className="font-semibold tabular-nums">{code ? `${code} ${money(local)}` : money(local)}</span></div>
        <AmountField label="Equivalent in Espees" required code="ES" error={err.espees_equivalent?.message} register={register} name={`${part}.${index}.espees_equivalent`} />
        {part === "sponsored" && <AmountField label="Espees already given" code="ES" error={err.espees_already_given?.message} register={register} name={`${part}.${index}.espees_already_given`} />}
      </div>
...
export function CrusadeExpenses() {
  ...
  return <div className="min-h-screen bg-[#f6f2e8] text-slate-950"><header className="border-b border-black/10 bg-white"><div className="mx-auto flex max-w-7xl items-center gap-3 px-5 py-4"><Link to="/"><img src="/logo.png" alt="A Night of a Thousand Crusades" className="h-11" /></Link><span className="hidden min-w-0 truncate text-sm font-semibold sm:block">Zonal Crusade Expense Report</span><Link to="/" className="ml-auto shrink-0 text-sm font-semibold">Return home</Link></div></header>
    ...
    <form onSubmit={handleSubmit(submit, () => toast.error("Check the highlighted details."))} className="bg-white px-5 py-12 sm:px-10 lg:px-14 lg:py-16"><div className="max-w-3xl"><p className="text-sm font-semibold text-amber-700">{editing ? `Editing · submitted ${editing.created_at.slice(0, 10)}` : "Zonal expense report"}</p><h2 className="mt-3 text-3xl font-medium leading-tight tracking-[-0.035em] sm:text-4xl">Crusade expenses for your zone</h2><p className="mt-4 max-w-2xl leading-7 text-slate-600">One consolidated report per zone, in two parts: the crusades NOTC sent you to, and the mega crusades your zone planned and held on its own.</p>
      ...
      <section className="mt-14"><div className="flex gap-3"><Plane className="mt-1 size-5 text-amber-600" /><div><p className="text-xs font-semibold uppercase tracking-[0.16em] text-amber-700">Part A</p><h3 className="mt-1 text-2xl font-medium tracking-[-0.02em]">Crusades we sent you to</h3><p className="mt-1 text-sm leading-6 text-slate-600">One entry per crusade: flights, what you already gave toward the sponsorship, and any other costs.</p></div></div>
        <ul className="mt-6 border-y border-black/15">{sponsored.fields.length === 0 ? <li className="py-8 text-sm text-slate-500">No crusades added yet.</li> : sponsored.fields.map((row, index) => <CrusadeCard key={row.id} part="sponsored" index={index} register={register} control={control} errors={errors} crusade={values.sponsored?.[index]} onRemove={() => removeCrusade("sponsored", index, sponsored)} files={filesFor("sponsored", index)} setFiles={setFilesFor("sponsored", index)} />)}</ul>
        <div className="flex flex-wrap items-center justify-between gap-4 py-5"><Button type="button" variant="outline" className="rounded-full transition-[transform,background-color] duration-150 active:scale-[0.96]" onClick={() => sponsored.append(emptySponsored())}><Plus /> Add a crusade we sent you to</Button><p className="text-sm text-slate-500">Part A total <span className="ml-2 font-semibold tabular-nums text-slate-950">ES {money(sponsoredEspees)}</span></p></div></section>

      <section className="mt-10"><div className="flex gap-3"><Coins className="mt-1 size-5 text-amber-600" /><div><p className="text-xs font-semibold uppercase tracking-[0.16em] text-amber-700">Part B</p><h3 className="mt-1 text-2xl font-medium tracking-[-0.02em]">Mega crusades you held on your own</h3><p className="mt-1 text-sm leading-6 text-slate-600">Crusades your zone planned and held itself — {MEGA_CRUSADE_MINIMUM.toLocaleString()} in attendance and above, not cellular. Venue and transportation, with evidence.</p></div></div>
        <ul className="mt-6 border-y border-black/15">{own.fields.length === 0 ? <li className="py-8 text-sm text-slate-500">No crusades added yet.</li> : own.fields.map((row, index) => <CrusadeCard key={row.id} part="own" index={index} register={register} control={control} errors={errors} crusade={values.own?.[index]} onRemove={() => removeCrusade("own", index, own)} files={filesFor("own", index)} setFiles={setFilesFor("own", index)} />)}</ul>
        <div className="flex flex-wrap items-center justify-between gap-4 py-5"><Button type="button" variant="outline" className="rounded-full transition-[transform,background-color] duration-150 active:scale-[0.96]" onClick={() => own.append(emptyOwn())}><Plus /> Add a crusade you held</Button><p className="text-sm text-slate-500">Part B total <span className="ml-2 font-semibold tabular-nums text-slate-950">ES {money(ownEspees)}</span></p></div>
        {partsError && <p className="text-xs font-medium text-red-700">{partsError}</p>}</section>

      <section className="py-10"><Field label="Notes" hint="Anything the administration should know — including crusades still coming up."><Textarea rows={4} {...register("notes")} /></Field><div className="mt-8 flex flex-col-reverse gap-4 border-t border-black/10 pt-8 sm:flex-row sm:items-center sm:justify-between"><p className="max-w-md text-xs leading-5 text-slate-500">Reopen the report with your zone and KingsChat username to add crusades or correct anything.</p><Button type="submit" disabled={isSubmitting} className="rounded-full bg-[#18234d] px-7 transition-[transform,background-color] duration-150 hover:bg-[#24346d] active:scale-[0.96]">{isSubmitting ? "Saving…" : editing ? "Update expense report" : "Submit expense report"}</Button></div></section>
    </div></form></main></div>;
}
