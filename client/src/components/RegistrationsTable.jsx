...
import { Eye, Pencil, Trash2, X, Search, Download, FileSpreadsheet, SlidersHorizontal, ChevronDown } from "lucide-react";
...
export function RegistrationsTable() {
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6">
      ...
      <section aria-label="Search and filter registrations" className="border-y border-slate-200 bg-white print:hidden">
        <div className="flex flex-col gap-3 p-4 sm:flex-row sm:items-center">
          ...
          <Button type="button" variant="outline" className="h-11 justify-between sm:min-w-40" aria-expanded={showFilters} aria-controls="registration-filters" onClick={() => setShowFilters((value) => !value)}><span className="flex items-center gap-2"><SlidersHorizontal /> Filters</span>{activeFilters.length > 0 && <span className="grid size-5 place-items-center rounded-full bg-blue-600 text-[11px] text-white">{activeFilters.length}</span>}<ChevronDown className={`size-4 transition-transform ${showFilters ? "rotate-180" : ""}`} /></Button>
        </div>
        {showFilters && <div id="registration-filters" className="grid gap-x-5 gap-y-4 border-t border-slate-200 bg-slate-50/60 p-4 sm:grid-cols-3 lg:grid-cols-4">
        ...
        {activeFilters.length > 0 && (
          <div className="flex flex-wrap items-center gap-2 border-t border-slate-200 px-4 py-3"><span className="mr-1 text-xs font-medium text-slate-500">Applied</span>
      ...
      <section aria-label="Registered crusade records" className="border-y border-slate-200 bg-white">
