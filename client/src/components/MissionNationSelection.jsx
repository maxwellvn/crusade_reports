...
const EMPTY = { pastor_name: "", zone_name: "", home_country_code: "", mission_country_code: "", contact_email: "", phone_country_code: "", phone_number: "", kingschat_username: "" };
...
function Receipt({ receipt }) {
  return <div className="min-h-screen bg-white">
    ...
    <main className="mx-auto max-w-3xl px-4 py-12 sm:px-6 sm:py-20">
      ...
      <p className="mt-5 max-w-2xl text-base leading-7 text-slate-600">This receipt confirms the zone's mission-nation preference. The final assignment will be made by the campaign administration.</p>
      <dl className="mt-10 border-y border-slate-200">{[
        ["Receipt", receipt.receipt_code], ["Zonal Pastor", receipt.pastor_name], ["Zone", receipt.zone_name],
        ["Zone home nation", receipt.home_nation], ["Preferred mission nation", receipt.mission_nation],
...
export function MissionNationSelection() {
  ...
  const zonesFetcher = React.useCallback(async (search) => {
    const rows = await getJSON("/zones");
    return rows.filter((row) => row.zone.toLowerCase().includes(search.toLowerCase())).map((row) => ({ value: row.zone, label: row.zone, sublabel: row.region }));
  }, []);
  ...
  return <div className="min-h-screen bg-white">
    <header className="border-b border-slate-200"><div className="mx-auto flex max-w-6xl items-center gap-3 px-4 py-4 sm:px-6"><Link to="/"><img src="/logo.png" alt="" className="h-11 w-auto" /></Link><span className="hidden text-sm font-semibold text-slate-950 sm:block">Mission Nation Selection</span><Link to="/" className="ml-auto text-sm font-semibold text-slate-700 hover:text-slate-950">Return home</Link></div></header>
    <main>
      <section className="border-b border-slate-200 bg-slate-950 text-white"><div className="mx-auto max-w-6xl px-4 py-12 sm:px-6 sm:py-20"><p className="text-sm font-semibold text-blue-300">A Night of a Thousand Crusades</p><h1 className="mt-4 max-w-4xl text-4xl font-normal leading-[1.02] tracking-[-0.035em] sm:text-6xl">Choose a nation. Lead 1,000 crusades.</h1><p className="mt-6 max-w-2xl text-base leading-7 text-slate-300 sm:text-lg">Zonal Pastors may express interest in one mission nation outside their zone's home nation. Each preference carries a proposed commitment of at least 1,000 crusades, with final assignments made by the campaign administration.</p></div></section>
      ...
      <form onSubmit={submit} className="mx-auto max-w-6xl px-4 py-10 sm:px-6 sm:py-14">
        <section className="grid gap-8 border-b border-slate-200 pb-12 lg:grid-cols-[18rem_minmax(0,1fr)] lg:gap-14">
          <div><p className="text-sm font-semibold text-blue-700">Zonal Pastor details</p><h2 className="mt-3 text-2xl font-medium tracking-[-0.025em] text-slate-950">Identify your zone.</h2><p className="mt-3 text-sm leading-6 text-slate-600">Your declared home nation will be excluded from the mission directory.</p></div>
          <div className="grid gap-5 sm:grid-cols-2">
            <Field label="Zonal Pastor name" htmlFor="mission-pastor-name" required error={errors.pastor_name}><Input id="mission-pastor-name" value={form.pastor_name} onChange={(e) => update("pastor_name", e.target.value)} aria-invalid={Boolean(errors.pastor_name)} placeholder="Full name" /></Field>
            <Field label="Zone" htmlFor="mission-zone" required error={errors.zone_name}><Combobox id="mission-zone" value={form.zone_name} fetcher={zonesFetcher} onSelect={(option) => update("zone_name", option.label)} placeholder="Select zone" searchPlaceholder="Search zones…" invalid={Boolean(errors.zone_name)} caps /></Field>
            <Field label="Zone home nation" htmlFor="mission-home-nation" required error={errors.home_country_code}><Combobox id="mission-home-nation" value={homeNation?.name || ""} fetcher={countriesFetcher} onSelect={(option) => update("home_country_code", option.value)} placeholder="Select home nation" searchPlaceholder="Search 242 nations…" invalid={Boolean(errors.home_country_code)} /></Field>
            ...
            <Field label="KingsChat username" htmlFor="mission-kingschat" required error={errors.kingschat_username}><Input id="mission-kingschat" value={form.kingschat_username} onChange={(e) => update("kingschat_username", e.target.value)} aria-invalid={Boolean(errors.kingschat_username)} placeholder="@username" /></Field>
          </div>
        </section>
        ...
        <aside className="grid gap-4 border-b border-slate-200 py-8 sm:grid-cols-[auto_1fr] sm:items-start">
          ...
          <div>
            ...
            <p className="mt-1 max-w-3xl text-sm leading-6 text-slate-600">Zonal Pastors should delegate local church tasks to Group Pastors so planning and execution can continue smoothly across every local church.</p>
          </div>
        </aside>

        <section className="py-12">
          <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between"><div><p className="text-sm font-semibold text-blue-700">Mission nation directory</p><h2 className="mt-3 text-3xl font-normal tracking-[-0.03em] text-slate-950">Select one mission nation.</h2><p className="mt-3 text-sm leading-6 text-slate-600">More than one zone may prefer the same nation. Your zone's home nation remains unavailable to you.</p></div>{catalogue && <div className="flex gap-6 text-sm"><p><span className="block text-2xl font-medium text-slate-950">{catalogue.total}</span><span className="text-slate-500">Nations</span></p><p><span className="block text-2xl font-medium text-slate-950">{catalogue.preferences}</span><span className="text-slate-500">Preferences</span></p></div>}</div>
