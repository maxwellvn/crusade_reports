  ...
  return <div className="min-h-screen bg-[#f5f7fb] text-slate-950">
    ...
    <main>
      ...
      <form onSubmit={handleSubmit(submit, () => toast.error("Complete the highlighted details and select one crusade."))} className="mx-auto max-w-7xl px-4 py-10 sm:px-6 sm:py-14">
        ...
        <section className="py-12"><div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between"><div><p className="text-sm font-semibold text-blue-700">Published schedule</p><h2 className="mt-3 text-3xl font-normal tracking-[-0.03em]">Select one upcoming crusade.</h2><p className="mt-3 max-w-2xl text-sm leading-6 text-slate-600">Choose your preferred crusade. Your selection will be highlighted before submission.</p></div><div className="relative w-full max-w-sm"><Search className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-slate-400" /><Input value={query} onChange={(event) => setQuery(event.target.value)} className="pl-9" placeholder="Search nation, city or crusade" /></div></div>
