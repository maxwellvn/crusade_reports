...
The admin surface borrows the layout structure of a modern analytics dashboard (KPI strip → main content grid → side rail) while keeping every visual token from the marketing system: monochrome neutrals, hairline dividers, no shadows beyond the softest card lift, `{rounded.full}` action pills.

### Page Structure
- **App background**: `{colors.canvas-warm}` — cards sit on it as `{colors.canvas}` panels with `{rounded.lg}` corners and a 1px `{colors.hairline-soft}` border (no drop shadows; the tonal step is the elevation).
- **Grid**: 12-column, `{spacing.lg}` (24px) gutters. Main column ≈ 8–9 cols, right rail ≈ 3–4 cols. Page padding `{spacing.xl}`.
- **Vertical rhythm**: `{spacing.lg}` between card rows — dashboards use the tight scale, never the marketing `{spacing.section}` gutters.

### Dashboard Components

**`kpi-strip`** — full-width stat banner at the top of the page
- One `{colors.canvas}` card containing 3–5 stat cells separated by 1px `{colors.hairline-soft}` vertical rules (a single slab, not floating cards — same move as the pricing slab).
- Each cell: label (`{typography.link-sm}` in `{colors.slate}`) → value (`{typography.display-sm}` in `{colors.ink}`, tabular figures) → delta line (`{typography.meta}`: ▲/▼ arrow + percentage in `{colors.graphite}`, supporting metric in `{colors.stone}`). No red/green — direction is carried by the arrow glyph, per the no-signal-colour rule.
- The first cell may carry an inline sparkline in `{colors.ink}` at 1.5px stroke with a 4% ink fill beneath.

**`panel`** — the generic dashboard card
- `{colors.canvas}` background, `{rounded.lg}`, 1px `{colors.hairline-soft}` border, padding `{spacing.lg}`.
- Header row: title (`{typography.heading-sm}` in `{colors.ink}`) left, controls right (tab set, dropdown, or close icon in `{colors.slate}`). A 1px `{colors.hairline-soft}` rule may separate header from body.

**`panel-tabs`** — in-card metric switcher (e.g. Pageviews / Clicks / Conversions)
- Text tabs in `{typography.link-sm}`: inactive `{colors.slate}`, active `{colors.ink}` with a 2px `{colors.ink}` underline. No pill fills, no accent colour.

**`chart-panel`** — primary time-series module (main column, widest card)
- Line/area chart drawn in `{colors.ink}` (1.5px stroke) over a gradient fill from 6% ink to transparent. Gridlines `{colors.hairline-soft}`, axis labels `{typography.meta}` in `{colors.stone}`.
- Hover state: vertical `{colors.ink}` rule + a `{colors.primary}` tooltip chip (`{colors.on-primary}` text, `{rounded.sm}`, `{typography.meta}`).

**`data-list`** — compact entity table (networks, sources, recent reports)
- Column headers in `{typography.micro-caps}` `{colors.stone}` over a 1px `{colors.hairline-soft}` rule; rows separated by the same rule, `{rounded.none}` cells.
- Row: 24px monochrome glyph/initial block (`{colors.hairline}` infill, `{rounded.sm}`) → name (`{typography.body}` `{colors.ink}`) → metric (`{typography.body}` `{colors.graphite}`, right-aligned, tabular) → trailing action (`{button-ghost}` compact at 32px height, or a `{typography.meta}` percentage).

**`side-rail-card`** — right-column summary modules (geography, entity detail, 30-day performance)
- Same `{panel}` shell. Key/value pairs: key `{typography.meta}` `{colors.stone}`, value `{typography.body-strong}` `{colors.ink}`.
- Progress meters are 4px bars: track `{colors.hairline}`, fill `{colors.ink}` (never a coloured fill).
- Status lines use a 16px monochrome icon + `{typography.body}` copy — state is worded ("Connection successful"), not coloured.

**`geo-map`** — interactive stats map (right rail or full-width panel)
- A dot-matrix world map inside a `{panel}`: land rendered as a grid of 2px dots in `{colors.stone}` at 40% opacity on the `{colors.canvas}` card — no country fills, no blue oceans. The map reads as texture until data lights it up.
- **Data markers**: locations with activity get a `{colors.ink}` dot scaled by magnitude (4–12px) with a 1px `{colors.canvas}` ring so overlapping markers stay legible. The top location may pulse once on load (a single 600ms scale-fade, not a loop).
- **Hover / tap interactivity**: hovering a marker (or a country's dot cluster) raises the dots to `{colors.graphite}` and opens a `{colors.primary}` tooltip chip — location name in `{typography.body-strong}` `{colors.on-primary}`, then 2–3 stat lines in `{typography.meta}` (e.g. "Attendance 12,400 · Reports 38"). Tooltip `{rounded.sm}`, padding `{spacing.xs}` × `{spacing.sm}`, follows the cursor.
- **Click**: selects the region — marker turns `{colors.primary}`, and the stat rows below the map (or the sibling `{data-list}`) filter to it. A `{tag-chip}` showing the active region with an × appears in the panel header; clicking it clears the filter.
- **Controls**: +/− zoom buttons top-left as 32px `{button-ghost}` squares (`{rounded.sm}`), and an optional region dropdown top-right in `{typography.link-sm}`.
- **Legend / summary strip**: below the map, 2–3 inline meters (`{side-rail-card}` progress-bar spec: 4px `{colors.hairline}` track, `{colors.ink}` fill) with `{typography.meta}` labels — the "at a glance" stats when nothing is hovered.
- Implementation note: an inline SVG map with vanilla JS hover/click handlers is sufficient — no mapping library needed for the dot-matrix style. Reach for Leaflet only if real pan/zoom tiles or street-level detail is ever required.

**`date-range-panel`** — calendar + preset picker
- Two-pane `{panel}`: calendar grid left, preset list right ("Today", "This week", "Last month"…, `{typography.body}` `{colors.graphite}`, active preset `{colors.ink}` `{typography.body-strong}`), separated by a 1px `{colors.hairline-soft}` rule.
- Day cells 40px, `{typography.body}`; out-of-month days `{colors.mute}`; selected day a `{colors.primary}` circle with `{colors.on-primary}` text; in-range days `{colors.hairline}` infill (the featured-tier move again).
- Footer: two bordered date fields (`{rounded.sm}`, 1px `{colors.hairline-soft}`) → arrow glyph → `{button-primary}` "Apply".

**`tag-chip`** — inline source/category tags in detail cards
- `{colors.hairline}` infill, `{colors.graphite}` text, `{typography.meta}`, padding `{spacing.xxs}` × `{spacing.sm}`, `{rounded.sm}`.

### Admin Do's and Don'ts
- Do keep exactly one `{button-primary}` per panel; every other in-panel action is `{button-ghost}` or `{button-text-link}`.
- Do render deltas and charts in the neutral ladder — direction comes from arrows and copy, not red/green.
- Don't float KPI stats as separate cards; the hairline-divided slab is the system's signature.
- Don't add drop shadows to panels — the `{colors.canvas}`-on-`{colors.canvas-warm}` step plus a hairline border is the only elevation.
